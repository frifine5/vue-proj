...
function handleResults2 (response) {
    ...
    if (remoteResponse.status === 200) {
        let rs = remoteResponse.data
        result.errorCode = rs.code
        ...
        result.errorCode = remoteResponse.status
    }
...
export default {
    ...
    uploadFileFormData (url, data, response, exception) {
        ...
        axios({
            ...
            (result) => {
                response( handleResults2(result) )   //handleResults(result))
            }
        ).catch(
            (error) => {
                if (exception) {
